...
.sampling <- function(Triangle, N=100, seuil = NA, zonnage=FALSE, stab=NA) {
  ...
  if(!is.na(stab)){
    ...
    prob.stab = !is.na(Triangle2) ################################################################################## p-e essayer avec prob.stab = 1 ici.
  }
    ...
    samples <- lapply(1:length(morceaux), function(i) {
      ...
      return(lapply(1:N, function(x) {
        matrix(sample(positions[, morceaux[[i]]],
          replace = TRUE,
          prob = prob*prob.stab[,morceaux[[i]]],
          size = length(prob)
        )
        ,
        nrow = I
...
BootMackChainLadder <- function(Triangle, B=100, distNy="normal", seuil=NA, zonnage=FALSE,BF.premiums=NULL,BF.param = c(5,5),stab=NA) {
  ...
  samples <- .sampling(residuals, B, seuil = seuil, zonnage = zonnage, stab = stab)
...
MultiBootMackChainLadder <- function(triangles, B=100, distNy = "normal", names=NULL, seuil=NA,stab=NA) {
